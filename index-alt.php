          ...
          <div class="section mt-4">
            ...
            <?php           
                ...
                if ( $posts->have_posts() ) :  while ( $posts->have_posts() ) : $posts->the_post(); 
                    get_template_part( 'youtube-content', get_post_format() );
                        endwhile; endif; wp_reset_postdata(); 
            ?>
            </p>
            <a href="<?php echo get_option('home'); ?>/youtube-archive/"  class="btn btn-sm btn-primary section-button">Submit Yours</a>
            <a href="<?php echo get_option('home'); ?>/youtube-archive/"  class="btn btn-sm btn-primary section-button">View the archives</a>
          </div>
        </div>
        <div class="col-lg-6 col-md-12 blog-posts">
        <?php   
            $recipe_cat_id = get_cat_id('recipes');         
            $args = array (
                'post_status' => 'publish',
                'post_type' => 'post',
                'order'          => 'desc',
                'orderby'        => 'publish_date',
                'posts_per_page' => 4,     
                'category__not_in' => array( $recipe_cat_id ),           
            );
